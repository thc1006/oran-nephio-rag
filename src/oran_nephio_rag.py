...
class SklearnTfidfEmbeddings:
    ...
    def __init__(self, max_features: int = 5000, model_name: str = "tfidf-sklearn") -> None:
        self.max_features = max_features
        self.model_name = model_name
        self.vectorizer = (
            TfidfVectorizer(max_features=max_features, stop_words="english", ngram_range=(1, 2), min_df=1, max_df=0.95)
...
class QueryProcessor:
    ...
    def __init__(self, config: Config, vector_manager: VectorDatabaseManager) -> None:
        self.config = config
        ...
        try:
            self.rag_manager = create_puter_rag_manager(model=config.PUTER_MODEL, headless=config.BROWSER_HEADLESS)
    ...
    @monitor_query("rag_query")  # 裝飾器用於監控查詢
...
請提供詳細而準確的回答："""
            ...
            if result.get("success"):
                return {"success": True, "answer": result.get("answer", ""), "method": "puter_js_browser"}
            else:
                return {
                    "success": False,
            ...
            return {"success": False, "error": str(e), "method": "puter_js_browser"}
    ...
    def _generate_fallback_answer(self, query: str, context_docs: List[str]) -> Dict[str, Any]:
        ...
        return {"success": True, "answer": answer, "method": "keyword_fallback"}
...
class ORANNephioRAG:
    ...
    def __init__(self, config: Optional[Config] = None) -> None:
        ...
        self.query_processor: Optional[QueryProcessor] = None  # 延遲初始化
    ...
    def query(self, question: str, **kwargs) -> Dict[str, Any]:
        ...
        if not self.is_ready:
            ...
            if not self.initialize_system():
                return {"success": False, "answer": "系統初始化失敗，請稍後再試。", "error": "system_not_ready"}

        if self.query_processor is not None:
            return self.query_processor.process_query(question, **kwargs)
        else:
            return {"success": False, "answer": "查詢處理器未初始化", "error": "processor_not_ready"}
    ...
    def get_system_status(self) -> Dict[str, Any]:
        ...
        status.update({"vectordb_ready": db_info.get("database_ready", False), "vectordb_info": db_info})
...
def quick_query(question: str, config: Optional[Config] = None, **kwargs) -> Dict[str, Any]:
    ...
    if not rag_system.initialize_system():
        return {"success": False, "answer": "系統初始化失敗。", "error": "initialization_failed"}
...
if __name__ == "__main__":
    ...
    if rag.initialize_system():
        ...
        # 示例查詢
        test_queries = ["什麼是 O-RAN？", "Nephio 如何部署網路功能？", "如何進行 O-RAN 的規模擴展？"]
