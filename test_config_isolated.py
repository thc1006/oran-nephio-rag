...
def run_isolated_tests(verbose: bool = True) -> Dict[str, Any]:
    ...
    cmd.extend(["-m", "not requires_heavy_deps and not requires_api_key and not requires_network"])
...
def main():
    ...
    if results["success"]:
        ...
        print(
            f"[INFO] Success Rate: {(results['tests_passed']/results['tests_run']*100):.1f}%"
