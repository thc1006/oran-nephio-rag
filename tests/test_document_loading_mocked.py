...
class TestDocumentLoaderMocking:
    ...
    @patch("src.document_loader.requests.Session")
    def test_document_loading_with_realistic_content(self, mock_session_class, sample_html_documents, mock_config):
        """Test document loading with realistic HTML content"""
        from src.config import DocumentSource
        from src.document_loader import DocumentLoader

        # Mock the response object with realistic content
        test_url = "https://docs.nephio.org/architecture"
        test_content = sample_html_documents["nephio_architecture"]
    ...
    def test_document_loading_error_scenarios(self, mock_config):
        ...
        # Test 404 error
        responses.add(responses.GET, "https://docs.nephio.org/notfound", status=404)
        ...
        assert doc is not None
        # Check for either fallback system (fallback_mode or is_sample) or that it's an actual error document
        assert (
            doc.metadata.get("fallback_mode") is True
            or doc.metadata.get("is_sample") is True
            or "Not Found" in doc.metadata.get("description", "")
        )
        assert (
            "Sample:" in doc.metadata.get("title", "")
            or "Sample -" in doc.metadata.get("title", "")
            or "Not Found" in doc.metadata.get("description", "")
        )
...
class TestDocumentMetadataMocking:
    ...
    def test_metadata_extraction(self, mock_config):
        ...
        assert (
            doc.metadata["meta_description"] == "Comprehensive guide for scaling O-RAN network functions using Nephio"
        )
        assert doc.metadata["source_url"] == source.url
        assert doc.metadata["source_type"] == "nephio"
...
class TestDocumentLoadingIntegration:
    ...
    @responses.activate
    def test_document_loading_to_vectordb_pipeline(
        self, mock_chromadb, mock_huggingface_embeddings, sample_html_documents, mock_config
    ):
