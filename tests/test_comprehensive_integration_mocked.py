...
class TestEndToEndRAGWorkflow:
    ...
    def test_complete_rag_pipeline(
        self, mock_all_external_services, sample_html_documents, sample_rag_query, sample_document_sources
    ):
...
class TestRAGSystemScaling:
    ...
    def test_concurrent_queries(self, mock_all_external_services):
        ...
        for i, query in enumerate(concurrent_queries):
            ...
            results.append(
                {"query_id": i, "query": query, "success": response["success"], "response_time": response["query_time"]}
            )
